...
### Phase 2（进行中）：拟真战斗
- [x] **控制区 + 借机攻击**：装备近战武器单位威胁周围 6 格，离开敌人控制区触发免费攻击；选中友方时显示敌方威胁地图（弱橙），鼠标悬停路径时实时高亮借机攻击触发点（橙红描边）
- [ ] 高低差（Elevation）
- [ ] 压制（Overwhelm）
- [ ] 士气/决心系统（六级士气状态）
- [x] **评分式 AI（BattleAI）**：枚举所有可达格 × 攻击目标，按评分函数选最优行动。评分项：命中×期望伤害、击杀奖励、低 HP 集火、规避借机攻击、避免落入多重 ZoC、节约移动
