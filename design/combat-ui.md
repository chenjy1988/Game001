...
| **待** | `Q` | 黄（0.85, 0.78, 0.30） | 第一次：排到本回合队尾（不消耗 AP）；第二次：结束回合（剩余 AP 作废） | ✅ 已实装 |
...
```
   ...
   点击红色目标    首次→队尾       回合结束
        ↓          二次→结束回合          ↓
   执行攻击 → IDLE  （剩余 AP 作废）   下回合
```

**AP 不足时**：chip 自动灰化，但仍可 hover 看 tooltip。
**已等待过**：Q chip 仍可用，tooltip 切换为「结束回合」；`TurnManager.can_wait()` 返回 true；第二次按下 → `end_turn()`。
...
## 十一、角色检视（Character Inspect）

> **设计 SoT**：[`CharacterInspect.md`](CharacterInspect.md)

| 层级 | 入口 | 用途 |
|---|---|---|
| 瞥视 | 鼠标悬停单位 | `UnitTooltip`：HP/甲/命中预览 |
| 深查 | **双击**单位（仅我方回合） | 全屏案牍：立绘 + 属性 + 装备 + 技能 + Buff + **随身道具** |
| 退出 | **Esc** | 回到战场，不消耗 AP |

- 打开检视时战场输入锁定；**不**替代 hover，**不做**战斗内固定 pin 卡。
- 底栏 `P` 详情条保留为「当前行动者」轻量视图；与检视数据同源，长期可收敛。
- 道具：**检视**列全身 `Unit.inventory`；**`I` 置物架**仅为 6 格战斗快捷栏（见 §十）。
